      ...
      >
        ...
        >
          ...
          {isAssignedToMe && ticket.status !== "CLOSED" && (
            <MenuItem
              onClick={() => {
                onStatusUpdate(ticket.id, "CLOSED");
                setMenuAnchor(null);
              }}
            >
...
export default function SupportAgentDashboard() {
  ...
  const { currentTickets, filteredTickets, stats } = useMemo(() => {
    ...
    const myAssignedTickets =
      tickets?.filter((t) => t.assignedAgent?.id === user.id) || [];
    const stats = {
      assigned: myAssignedTickets.length,
      open: myAssignedTickets.filter((t) => t.status === "OPEN").length,
      inProgress: myAssignedTickets.filter((t) => t.status === "IN_PROGRESS")
        .length,
        ...
        <DialogContent>
          {ticketDetails && (
            <Stack spacing={2}>
              ...
              <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap" }}>
                <Chip
                  label={`Status: ${ticketDetails.status}`}
                  color="primary"
                />
                <Chip
                  label={`Priority: ${ticketDetails.priority}`}
                  variant="outlined"
                />
                <Chip
